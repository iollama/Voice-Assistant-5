...
- **Speaker (MAX98357A and/or optional PCM5102A):** `I2S_NUM_1`, master TX, 24kHz, 16-bit samples, `I2S_CHANNEL_FMT_ONLY_LEFT` (mono — 1 sample = 1 LRCLK period). MAX98357A in stereo-average mode plays this on the speaker; the PCM5102A jack receives audio on the left channel only. `tx_desc_auto_clear = true` prevents DMA underrun noise. The pipeline payload is mono PCM16 throughout; a stereo channel format here would require mono→stereo expansion in `manageSpeaker()` before `i2s_write()` (deferred — left-only is sufficient for AUX-in on most powered speakers).

#### Dual-DAC layout (PCB builds)

When the PCB is built with both chips populated, they share BCLK / LRCLK / DOUT and the firmware mutes whichever is not selected via two extra GPIOs:
...
Both QR codes are generated by `render_qr()` using the `esp_qrcode` API bundled with arduino-esp32 — `esp_qrcode_generate()` runs a synchronous `display_func` callback that walks modules via `esp_qrcode_get_module()` and draws them with `gfx->fillRect()`. The callback closes over three file-scope statics (`s_qr_module_px` / `s_qr_x_center` / `s_qr_y_top`) since the espressif API doesn't pass user-data through. Safe because `render_qr` only runs from Core 1's `loop()` (display lives on Core 1) and the generate call is synchronous. One-time generation at display time, no heap allocations on the render path.
